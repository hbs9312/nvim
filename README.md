...
| 단축키 | 모드 | 설명 |
|--------|------|------|
| `<C-,>` | Normal/Terminal | Claude Code 터미널 토글 |
| `<leader>ac` | Normal | Claude Code 터미널 토글 |

### 변형 명령
| 단축키 | 모드 | 설명 |
|--------|------|------|
| `<leader>cC` | Normal | 최근 대화 재개 (`--continue`) |
| `<leader>cV` | Normal | 상세 로깅 활성화 (`--verbose`) |

### 터미널 내 윈도우 이동
| 단축키 | 모드 | 설명 |
|--------|------|------|
| `<C-h>` | Terminal | 왼쪽 윈도우로 이동 |
| `<C-j>` | Terminal | 아래 윈도우로 이동 |
| `<C-k>` | Terminal | 위 윈도우로 이동 |
| `<C-l>` | Terminal | 오른쪽 윈도우로 이동 |

### 터미널 내 스크롤
| 단축키 | 모드 | 설명 |
|--------|------|------|
| `<C-f>` | Terminal | 한 페이지 아래로 스크롤 |
| `<C-b>` | Terminal | 한 페이지 위로 스크롤 |

> **참고**: 스크롤 후 `i` 키를 눌러 insert 모드로 돌아가 Claude Code와 계속 상호작용할 수 있습니다.
